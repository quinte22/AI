...
def readCommand(argv):
    ...
    parser.add_option('--graphics',
                    dest = 'noGraphics',
                    action = 'store_false',
                    help = 'graphics display for pacman games.')
...
def getDisplay(graphicsByDefault, opt=None):
    graphics = graphicsByDefault
    if opt is not None and options.noGraphics:
        graphics = False
    if opt is not None and not options.noGraphics:
        graphics = True
    if graphics:
        try:
