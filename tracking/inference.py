...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        Pr(noisyDistance | Xt=p) =
            Pr(noisyDistan ce | manhattanDistance(p,packmanPosition))
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                    # since we are trying to solve when Pr(Xt=p |e_t, e_{t-1}, e_{t-2}, ..., e_1)
                    # and we have Pr(Xt=p |e_{t-1}, e_{t-2}, ..., e_1) we know
                    # Pr(Xt=p |e_t, e_{t-1}, e_{t-2}, ..., e_1) = Pr(Xt=p |e_{t-1}, e_{t-2}, ..., e_1)*Pr(noisyDistance | Xt=p) where noisyDistance is e_t
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            new = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for new_postion in new.keys():
                all_possible[new_postion] = all_possible[new_postion] + new[new_postion] * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        "*** END YOUR CODE HERE ***"
        # print "legal board positions {}".format(self.legalPositions)
        num_of_particles = self.numParticles/len(self.legalPositions)
        # print("num per particale {}".format(num_of_particles))
        final_list = [self.legalPositions[k] for i in range(num_of_particles) for k in range(len(self.legalPositions))]
        # print("the len {}".format(len(final_list)))
        # print("num of dist {}".format(len(set(final_list))))
        # print "final list {}".format(final_list)
        # print "num_of_particles {}".format(self.numParticles)
        prob = 1./float(len(self.legalPositions))
        self.variables = final_list
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.variables = [self.getJailPosition()] * self.numParticles
        else:
            all_possible = util.Counter()
            old_belief = self.getBeliefDistribution()
            for pos in self.legalPositions:
                d = util.manhattanDistance(pos, pacmanPosition)
                all_possible[pos] = all_possible[pos] + (
                            emissionModel[d] * old_belief[pos])
            if sum(all_possible.values()) == 0:
                self.initializeUniformly(gameState)
            else:
                temp = []
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.variables:
            new = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            new_pos.append(new)
        for p in range(len(new_pos)):
            self.variables[p] = util.sample(new_pos[p])

        #     for new_postion in new.keys():
        #         all_possible[new_postion] = all_possible[new_postion] + new[new_postion] * self.getBeliefDistribution[p]
        # self.beliefs = all_possible
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # take list and find diestribution
        all_possible = util.Counter()
        for p in list(set(self.variables)):
            all_possible[p] = len([k for k in self.variables if p == k])
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # same thing as before.. Just each particales has ghost
        # there are n number of ghost. n! possible
        final_list = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        ...
        for p in range(len(self.particles)):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    d = util.manhattanDistance(self.particles[p][i], pacmanPosition)
                    start *= emissionModels[i][d]
            all_possible[self.particles[p]] += start
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for g in range(self.numGhosts):
                new = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), g, self.ghostAgents[g])
                newParticle[g] = util.sample(new)
